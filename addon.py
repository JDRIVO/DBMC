...
def run():
    ...
    if HANDLE < 0:
        ...
        # Handle action of a file or a "Load more" item
        if "media_items" in params:
            # Loading more media items requested
            path = ADDON_URL + PARAMS
            # xbmc.executebuiltin(f"Container.update({path}, replace)") # Don't use replace because that removes the content_type from the path
